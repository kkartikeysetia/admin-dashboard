...
/* eslint-disable prefer-template, comma-dangle, react/button-has-type, nonblock-statement-body-position, curly */
import React, { useState, useMemo, useEffect } from "react";
import { ordersData } from "../data/dummy";
                ...
                <thead className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                  <tr>
                    {[
                      ...
                      <th
                        key={col.key}
                        className="text-left py-4 px-6 font-semibold text-slate-700 cursor-pointer hover:bg-slate-
WHITE"
                        onClick={() => handleSort(col.key)}
                      >
                        <span className="flex items-center gap-1">
            ...
            {/* Pagination */}
            <div className="px-6 py-4 border-t border-slate-200 bg-gradient-to-r from-slate-50 to-indigo-100">
              <div className="flex items-center justify-between">
                <div className="text-sm text-slate-600">
                  Showing {(currentPage - 1) * itemsPerPage + 1} to{" "}
                  {Math.min(
